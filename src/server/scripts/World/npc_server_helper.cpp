...
#define EMOTE_NO_SICKNESS "|CFFFFCCFF You Dont Have Resurrection Sickness Aura!|r"
#define EMOTE_NO_DESERTER "|CFF00CCFF You Dont Have Deserter Aura!|r"
#define EMOTE_COOLDOWN "Sorry My Friend, Can Not Do It This Time."
#define EMOTE_NO_ARENA_POINTS "|CFFCC00FF You Have Enough Arena Points!|r"
#define EMOTE_TELEPORT_ONLY_HORDE "|CFFFF0000 This Teleportation Is Allowed Only To Horde!|r"
#define EMOTE_TELEPORT_ONLY_ALLIANCE "|CFF0000FF This Teleportation Is Allowed Only To Alliance!|r"
#define EMOTE_NO_ENOUGH_EMBLEMS "|CFF00FF00 You Have Enough Emblems!|r"
#define EMOTE_NO_MONEY "|CFFFFFFCC You Dont Have Enough Money!|r"
#define EMOTE_FUNCTION_OFF "|CFF9933FF Function Disable!|r"
...
enum Actions
{
	...
	ACTION_DEMORPH                       = 564,
	// Event Action
	ACTION_ENTER_EVENT                   = 565,
	ACTION_LEAVE_EVENT                   = 566,
};
...
public:
	...
	bool OnGossipHello(Player* player, Creature* creature)
	{
		...
		if (ConfigMgr::GetBoolDefault("Server.Helper.Enable", true))
		{
			...
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TABARD, "Event Menu ->", GOSSIP_SENDER_MAIN, EVENT_MENU);
	...
	bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action)
	{
		...
		case BUFF_MENU:
			if (ConfigMgr::GetBoolDefault("Server.Helper.Buff.Menu.Enable", true))
			{
				...
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "Arcane Focus", GOSSIP_SENDER_MAIN, ACTION_ARCANE_FOCUS);
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, "<- [Main Menu]", GOSSIP_SENDER_MAIN, MAIN_MENU);
				player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
				return true;
			}
		...
		case ACTION_WATER_BREATHING:
			...
			break;
		case ACTION_ARCANE_FOCUS:
			player->CLOSE_GOSSIP_MENU();
			player->CastSpell(player, SPELL_ARCANE_FOCUS, true);
			break;
		case CUSTOMIZE_MENU:
		...
		case MAIN_MENU:
			...
			break;
		case EVENT_MENU:
			if (ConfigMgr::GetBoolDefault("Server.Helper.Event.Menu.Enable", true))
			{
				player->PlayerTalkClass->ClearMenus();
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "Teleport Me To Event [25.000 Gold]", GOSSIP_SENDER_MAIN, ACTION_ENTER_EVENT);
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Leave Event ->", GOSSIP_SENDER_MAIN, ACTION_LEAVE_EVENT);
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, "<- [Main Menu]", GOSSIP_SENDER_MAIN, MAIN_MENU);
				player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE,creature->GetGUID());
				return true;
			}
